...
class NBodyPanel : JPanel() {
    ...
    fun defaultBodies(): MutableList<Body> {
        val s = 150.0  // s — модуль «дрейфовой» скорости диска (px/сек), задаёт движение всего диска целиком
        ...
        val disc1 = BodyFactory.makeKeplerDisk(
            ...
            r = Config.R/2,                     // r — радиус диска (макс. расстояние частиц от центра) в пикселях
            clockwise = false              // clockwise — вращение по часовой
        )

        val disc0 = BodyFactory.makeKeplerDisk(
            nTotal = Config.N * 2,                 // nTotal — сколько тел создать в этом диске (включая центральное)
            vx = 0.0,                        // vx — добавочный сдвиг скорости по X для всех тел диска (px/сек)
            vy = 0.0,                      // vy — добавочный сдвиг скорости по Y для всех тел диска (px/сек)
            x = Config.WIDTH_PX * 0.5,     // x — координата центра диска по X (в пикселях экрана)
            y = Config.HEIGHT_PX * 0.5,    // y — координата центра диска по Y (в пикселях экрана)
        ...
        val disc2 = BodyFactory.makeKeplerDisk(
            ...
            r = Config.R/2,                      // радиус второго диска
            clockwise = false
        )

        // Склеиваем оба списка тел в один
        return (disc1 + disc2 + disc0).toMutableList()
    }
